...
class ProductTemplate(models.Model):
    ...
    def _get_additionnal_combination_info(self, product_or_template, quantity, date, website):
        ...
        if self.hide_sales_price and not combination_info.get('prevent_zero_price_sale'):
            combination_info.update({'prevent_zero_price_sale': True})
        return combination_info

    def _get_sales_prices(self, pricelist, fiscal_position):
        res = super(ProductTemplate, self)._get_sales_prices(
            pricelist, fiscal_position
        )
